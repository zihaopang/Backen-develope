...
# 文件传输协议：FTP
	用户首先提供远程主机的主机名，使得本地主机的FTP客户端建立一个到远程主机的连接。随后该用户接着提供用户标识和口令在TCP上传送。一旦该服务器向用户授权，则进行传送。
	Http和FTP的区别：
...
### 3.邮件访问协议
在以前，接收方是通过直接登陆服务器主机来完成的。而在现在用户通过在用户端系统上运行的客户端程序来阅读电子邮件。用户在本地PC上运行一个用户代理程序，访问总是开机的邮件服务器。所以，一般的过程为：
...
DNS是：
	- 一个由分层路由器实现的分布式数据库
	- 一个使主机能够查询数据库的协议
当某个主机用户上的浏览器请求URL: www.baidu.com/index.html 会发生什么现象
	- 用户主机上运行着DNS应用服务端
...
*DNS查询的简单例子*
	- 假设理工大学（cis.poly.edu）的本地DNS服务器为：dns.poly.edu,想知道gaia.cs.umass.edu的IP地址
	- gaia.cs.umass.edu的权威服务器地址为dns.umass.edu
...
### 2.分布式散列表
考虑如何在P2P网络中实现一个简单的数据库，即分布式数据库，也被称为分布式散列表（Distributed Hash Table,DHT）.
